...
def charger_donnees_uploaded(file):
    donnees = pd.read_csv(file, encoding='iso-8859-1', sep=';', dtype=spec_types, low_memory=False)
    donnees['Date dépôt GED'] = pd.to_datetime(donnees['Date dépôt GED'], format='%d/%m/%Y', errors='coerce')
    return donnees
...
# Téléchargement de fichiers par l'utilisateur
...
if uploaded_files:
    ...
    donnees = projets[projet_selectionne]
