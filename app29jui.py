...
def afficher_graphique(selectionne, donnees, projets, projet_selectionne):
    ...
    # Onglet 8: Calendrier des Projets
        ...
        donnees_gantt['Types de documents'] = donnees_sorted.groupby(categorie_gantt)['TYPE DE DOCUMENT'].apply(lambda x: ', '.join(x.unique())).reset_index(drop=True)
        ...
        fig_gantt.update_traces(
            hovertemplate=f'<b>{categorie_gantt}:</b> %{{y}}<br><b>Date Fin:</b> %{{x|%d %b %Y}}<br><b>Durée:</b> %{{customdata[0]}} jours<br><b>Nombre de documents:</b> %{{customdata[1]}}<br><b>Types de documents:</b> %{{customdata[2]}}'
        )
