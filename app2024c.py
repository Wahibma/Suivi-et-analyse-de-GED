...
def charger_donnees(projet_selectionne):
    return pd.read_csv(projets[projet_selectionne], encoding='iso-8859-1', sep=';', dtype=str)

# Configuration de l'application Streamlit
st.sidebar.title("Navigation")
onglets = ["Répartition des documents", "Evolution des indices", "Indicateur de Récapitulatif d'Alerte"]
selection_onglet = st.sidebar.radio("Aller à", onglets)

# Fonction pour créer un graphique de répartition des documents
def creer_graphique_repartition(df, colonne, titre):
    distribution = df[colonne].value_counts().reset_index()
    distribution.columns = [colonne, 'Nombre']
    fig = px.pie(distribution, values='Nombre', names=colonne, title=titre)
    fig.update_traces(textposition='inside', textinfo='percent+label')
    return fig

# Fonction pour créer un graphique d'évolution des indices
def creer_graphique_evolution(df, colonne, titre):
    df['Date'] = pd.to_datetime(df['Date'], errors='coerce')
    evolution = df.groupby([pd.Grouper(key='Date', freq='M'), colonne]).size().reset_index(name='Nombre')
    fig = px.line(evolution, x='Date', y='Nombre', color=colonne, title=titre)
    return fig

# Gestion de l'affichage selon l'onglet sélectionné
if selection_onglet == "Répartition des documents":
    st.title("Répartition des documents")
    projet_selectionne = st.selectbox("Sélectionnez un projet", list(projets.keys()))
    df = charger_donnees(projet_selectionne)
    categorie_selectionnee = st.selectbox("Sélectionnez une catégorie", df.columns)
    st.plotly_chart(creer_graphique_repartition(df, categorie_selectionnee, f"Répartition des documents par {categorie_selectionnee}"), use_container_width=True)

elif selection_onglet == "Evolution des indices":
    st.title("Evolution des indices")
    projet_selectionne = st.selectbox("Sélectionnez un projet", list(projets.keys()))
    df = charger_donnees(projet_selectionne)
    indice_selectionne = st.selectbox("Sélectionnez un indice", df.columns)
    st.plotly_chart(creer_graphique_evolution(df, indice_selectionne, f"Evolution des indices par {indice_selectionne}"), use_container_width=True)

elif selection_onglet == "Indicateur de Récapitulatif d'Alerte":
    st.title("Indicateur de Récapitulatif d'Alerte")
    # Le code pour cette section est déjà intégré plus haut
