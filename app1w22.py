...
def afficher_menu():
    with st.sidebar:
        selectionne = option_menu(
            menu_title="Menu",
            options=["Flux des documents", "Évolution des types de documents", "Analyse des documents par lot et indice", "Identification des acteurs principaux", "Analyse de la masse de documents par projet", "Nombre d'indices par type de document", "Durée entre versions de documents", "Diagramme de Gantt séquentiel"],
...
def afficher_graphique(selectionne, donnees, projets, projet_selectionne):
    ...
    # Onglet 7: Durée entre versions de documents
        ...
        if representation == "Tableau":
            if type_calcul == 'mean':
                resultats = donnees.groupby(categorie)['Différence en jours'].mean().reset_index()
                resultats.columns = [categorie, 'Durée moyenne entre versions (jours)']
            elif type_calcul == 'max':
                resultats = donnees.groupby(categorie)['Différence en jours'].max().reset_index()
    ...
    # Onglet 8: Diagramme de Gantt séquentiel
    elif selectionne == "Diagramme de Gantt séquentiel":
        st.header("Diagramme de Gantt des Projets")
        ...
        fig_gantt = px.timeline(
            ...
            hover_data=['Durée en jours', 'Nombre de documents'],
            color_discrete_sequence=couleurs,
            title=f'Diagramme de Gantt des Projets par {categorie_gantt}'
        )
        ...
        fig_gantt.update_traces(
            hovertemplate=f'<b>{categorie_gantt}:</b> %{{y}}<br><b>Début:</b> %{{x|%d %b %Y}}<br><b>Durée:</b> %{{customdata[0]}} jours<br><b>Nombre de documents:</b> %{{customdata[1]}}'
        )
